...
class WorkflowExecutionSerializerV1(BaseModel):
    """
    WorkflowExecutionSerializerV1
    """
    id: StrictStr = Field(...)
    type: constr(strict=True, max_length=256) = Field(..., description="The workflow type.")
    status: StrictStr = Field(..., description="Status of the workflow execution.  * `PENDING` - Pending * `RUNNING` - Running * `COMPLETED` - Completed * `FAILED` - Failed * `CANCELED` - Canceled * `TERMINATED` - Terminated * `CONTINUED_AS_NEW` - Continued as new * `TIMED_OUT` - Timed out")
    data: Dict[str, Any] = Field(..., description="The workflow execution input data.")
    result: Dict[str, Any] = Field(..., description="The workflow execution result.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> WorkflowExecutionSerializerV1:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> WorkflowExecutionSerializerV1:
        """Create an instance of WorkflowExecutionSerializerV1 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return WorkflowExecutionSerializerV1.parse_obj(obj)

        _obj = WorkflowExecutionSerializerV1.parse_obj({
            "id": obj.get("id"),
            "type": obj.get("type"),
